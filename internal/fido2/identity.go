...
// x25519Identity implements age.Identity for FIDO2-derived keys
...
// x25519Recipient implements age.Recipient for FIDO2-derived keys
...
func (id *Identity) ToAgeIdentity(devicePath, pin string) (age.Identity, error) {
	privKey, err := id.DerivePrivateKey(devicePath, pin)
	if err != nil {
		return nil, err
	}
...
func (id *Identity) ToAgeRecipient() age.Recipient {
	return &x25519Recipient{publicKey: id.PubKey}
}
